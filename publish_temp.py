...
loop_count = 0
max_temp = 85
# 5 minute wait between alerts
back_off = 5 * 60 

while True:
    ts = round(time.time())
    dt = datetime.fromtimestamp(ts)
    t = '{}:{:02d} {}/{}/{}'.format(dt.hour, dt.minute, dt.month, dt.day, dt.year)
    temp = round(tempprobe.get(), 1)

    if temp >= max_temp and loop_count == 0:
        message = {}
        message['time'] = t
        ...
        loop_count += 1
    elif temp >= max_temp - 1 and loop_count > 0:
        loop_count = (loop_count + 1) % back_off
        print(loop_count)
    else:
        loop_count = 0
